...
* **Pick a Site's PHP Version from the Dashboard:** `cove php` gained a UI. Every row's action menu now shows the site's effective PHP version and opens a picker listing Cove's default (FrankenPHP) alongside every Homebrew PHP the machine could run — installed versions switch with one click, versions that aren't installed yet are greyed out but still clickable: choosing one kicks off the `brew install` in the background and pins the site the moment it lands, with the dashboard polling quietly and confirming when the site is live on the new version. Sites pinned off the default wear a small `php 8.2`-style chip in the list so a mixed fleet reads at a glance, and a pin that lands mid-server-reload is retried idempotently the same way renames are. Versions that left Homebrew core (8.0, 7.4) appear only when a tap already provides them, so the installer never offers something it can't deliver.
