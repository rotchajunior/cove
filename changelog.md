...
* **`cove pull` Falls Back to SSH When the Backup URL Isn't Reachable:** The download URL is built by the remote from its own `home` option, so it only resolves when the remote's WordPress URL is publicly reachable *and* maps to the directory holding the backup. That quietly fails for staging sites behind basic auth or an IP allowlist, hosts on a private network, and sandboxes whose `home` still points at a provisioning template. Cove now checks the URL first and, when it doesn't resolve, streams the backup down the SSH connection it already holds — landing it where the migration step picks it up as a local file instead of re-downloading. Pulls that used to die with "Download failed" now just work.

### 🔒 Security & Bug Fixes

* **`cove pull` No Longer Empties the Database It's About to Restore Into:** Overwriting an existing site ran a `DROP DATABASE` / `CREATE DATABASE` before invoking the migration, but the migration's own preflight requires the destination to be an installed WordPress — and an emptied database fails that check with "WordPress not found in current directory". The restore aborted having already destroyed the local database, leaving the site with no tables at all. The pre-emptive drop was redundant to begin with: the migration runs `wp db reset` immediately before importing, so the clean slate was already guaranteed at the right moment. The local site now stays intact and servable right up until the restore actually succeeds.
